...
The skill's `config.json` accepts two optional token fields:

```json
{
  "api_token": "<multi-scope or single-scope>",
  "api_token_single_scope": "<single-account-pinned>"
}
```
...
`S1_IGW_URL`), or by setting `uam_alert_interface_url` in `config.json`
