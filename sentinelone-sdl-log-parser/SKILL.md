...
description: Use whenever the user wants to author, edit, debug, validate, or explain a SentinelOne Singularity Data Lake (SDL) log parser — the augmented-JSON definition files stored at /logParsers/<name> that extract fields from raw log text before ingestion. Trigger on "SDL parser", "Skylight parser", "log parser", "parser editor", "parser definition", "create a parser for <vendor>", "write a parser for this log", "why isn't my parser extracting field X", "test parser", "parser tester", or any pasted raw log line that the user wants turned into structured fields. Also trigger on parser-DSL keywords like `formats:`, `patterns:`, `lineGroupers:`, `rewrites:`, `discardAttributes:`, `attributes:`, `aliasTo:`, `mappings:`, `$fieldName$`, `{parse=...}`, `{regex=...}`, `halt: true`, `repeat: true`, `intermittentTimestamps`, `skipNumericConversion`, and `computeFields`. Especially trigger when the user pastes a raw log sample (CEF, syslog, JSON line, key=value, multi-line stack trace, CSV, anything ad-hoc) and asks to extract fields, normalize timestamps, tag with constants, drop noise, mask sensitive values, or split sub-structures (URLs, embedded JSON, base64). Use even when the user just says "parse this log" without naming SDL — if the surrounding project is SDL/Singularity/Scalyr, this is the right skill. After authoring, this skill always validates end-to-end via the sentinelone-sdl-api skill (putFile → uploadLogs → query) against a live tenant. NOT for writing PowerQuery hunts (use sentinelone-powerquery), NOT for ingesting events without a parser (use sentinelone-sdl-api directly).
---
